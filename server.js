...
app.get('/', (req, res) => {
  res.json({ status: 'ok', service: 'Green Leaf AI Receptionist is running' });
});
...
app.post('/gather-input', async (req, res) => {
  ...
  try {
    ...
    if (shouldEnd && Object.keys(session.leadData).length > 0) {
      ...
      twiml.say(
        { voice: 'alice' },
        'Thank you for calling Green Leaf. We have your information and someone will contact you soon. Goodbye!'
      );
      twiml.hangup();
    ...
    twiml.say(
      { voice: 'alice' },
      'Sorry, I had trouble understanding that. Could you please repeat?'
    );
...
function extractLeadData(session, userInput) {
  const input = userInput.toLowerCase();
  
  // Extract name pattern (very basic, in production use NLP)
  if (input.includes('my name is') || input.includes('i\'m') || input.includes('i am')) {
    const nameMatch = userInput.match(/(?:my name is|i'm|i am)\s+([a-zA-Z\s]+)/i);
    if (nameMatch) {
      session.leadData.name = nameMatch[1].trim();
    }
  }
  ...
  if (phoneMatch) {
    session.leadData.phone = phoneMatch[0];
  }

  // Extract email pattern
  const emailMatch = userInput.match(/[\w.-]+@[\w.-]+\.\w+/);
  if (emailMatch) {
    session.leadData.email = emailMatch[0];
  }

  // Track service type
  if (input.includes('cleaning')) {
    session.leadData.service = 'Cleaning';
  } else if (input.includes('construction')) {
    session.leadData.service = 'Construction';
  }

  // Track date/time mentions
  if (input.match(/\b(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/i)) {
    const dayMatch = userInput.match(/\b(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/i);
    session.leadData.preferredDay = dayMatch[0];
  }

  if (input.match(/\b([0-9]{1,2}):([0-9]{2})\s*(am|pm)\b/i)) {
    const timeMatch = userInput.match(/\b([0-9]{1,2}):([0-9]{2})\s*(am|pm)\b/i);
    session.leadData.preferredTime = timeMatch[0];
  }
}
...
async function sendLeadEmail(leadData) {
  const emailContent = `
New Lead from Green Leaf AI Receptionist:

${leadData.name ? `Name: ${leadData.name}` : ''}
${leadData.phone ? `Phone: ${leadData.phone}` : ''}
${leadData.email ? `Email: ${leadData.email}` : ''}
${leadData.service ? `Service Interested: ${leadData.service}` : ''}
${leadData.preferredDay ? `Preferred Day: ${leadData.preferredDay}` : ''}
${leadData.preferredTime ? `Preferred Time: ${leadData.preferredTime}` : ''}
